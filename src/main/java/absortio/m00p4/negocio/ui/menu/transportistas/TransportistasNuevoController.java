...
@Component
public class TransportistasNuevoController implements Initializable {

    @FXML
    Pane paneNuevoTransportista;
...
// ---------------------------------------------------
    ...
    void cargarComboBoxTipoDoc(){
        ArrayList<String> lTipoDoc = new ArrayList<>();
        lTipoDoc.add("DNI");
        lTipoDoc.add("RUC");
        //lTipoDoc.add("CE"); // carnet de extranjeria
        //lTipoDoc.add("PA"); // pasaporte

        ObservableList<String> olTipoDoc = FXCollections.observableArrayList(lTipoDoc);

        comboBoxTipoDocumento.getItems().addAll(olTipoDoc);
        comboBoxTipoDocumento.setValue(olTipoDoc.get(0));
        comboBoxTipoDocumento.setEditable(true);

        TextFields.bindAutoCompletion(comboBoxTipoDocumento.getEditor(), comboBoxTipoDocumento.getItems());
    }
    ...
    public void cambiarOpciones(){
        if (tipoDocumento.compareTo("RUC") == 0) {
            ...
            textFieldApellidoPat.setVisible(true);      // Mostramos
            textFieldApellidoMat.setVisible(true);
    ...
    @FXML
    public void comboBoxSeleccionTipoDoc() {
        tipoDocumento = comboBoxTipoDocumento.getValue().toString();
        cambiarOpciones();
    }
    ...
    // Volvemos a la pantalla anterior
        Pane paneParent = (Pane) paneNuevoTransportista.getParent();    // Con esto obtengo la pantalla base "menuform.fxml"
    ...
    void guardarDato (){
        ...
        transportistaModel.setEstado("activo");
        transportistaService.save(transportistaModel);        // Guardamos el nuevo valor
    }
